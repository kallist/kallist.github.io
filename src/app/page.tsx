...
function ChapterMark({ number, title }: { number: string; title: string }) {
  return <div className="v2-chapter-mark v2-reveal"><span>{number} / {title}</span><span aria-hidden="true">+ — + — + — +</span></div>;
}
  ...
  return <main id="main" className="v2-home">
    ...
    <section id="hero" className="v2-hero v2-field-host" aria-labelledby="hero-title">
      ...
      <div className="v2-frame">
        <div className="v2-hero-top v2-meta"><span>Independent practice / 2026</span><span>AI systems × Visual thinking</span></div>
        <div className="v2-hero-grid">
          <div className="v2-hero-name v2-reveal">
            <span className="v2-index">00 / Introduction</span>
            <h1 id="hero-title">kallist<span aria-hidden="true">.</span></h1>
        ...
        <div className="v2-hero-bottom v2-meta"><span>Context / Agents / Evidence / Art</span><Link href="#work">Scroll to selected work <span aria-hidden="true">↓</span></Link></div>
      </div>
    </section>
    ...
    <section id="work" className="v2-work" aria-labelledby="work-title">
      <div className="v2-frame"><ChapterMark number="01" title="Selected work" />
        <div className="v2-section-intro v2-reveal"><h2 id="work-title">Selected <em>work.</em></h2><p><WordHover text="Four systems. Four different boundaries. Each case keeps its source, decisions and limits one click away." /></p></div>
      </div>
      <article className="v2-case v2-field-host" aria-labelledby="repobound-title">
        <SectionPattern kind="repobound" />
        <div className="v2-frame v2-case-inner">
          <div className="v2-case-count"><span>01 / 04</span><span>{repobound.kicker}</span></div>
          <div className="v2-case-copy v2-reveal"><h3 id="repobound-title">Repo<span>Bound</span></h3><p className="v2-case-lead"><WordHover text={repobound.summary} /></p><p><WordHover text={repobound.problem} /></p><CaseLink slug={repobound.slug} /></div>
          <div className="v2-case-media v2-reveal"><ProjectImage src={repobound.image} alt={repobound.imageAlt} caption={repobound.imageCaption} /></div>
        </div>
      </article>
      <article className="v2-case v2-case-dark v2-case-reverse v2-field-host" aria-labelledby="cueparcel-title">
        <SectionPattern kind="cueparcel" />
        <div className="v2-frame v2-case-inner">
          <div className="v2-case-count"><span>02 / 04</span><span>{cueparcel.kicker}</span></div>
          <div className="v2-case-copy v2-reveal"><h3 id="cueparcel-title">Cue<em>Parcel</em></h3><p className="v2-case-lead"><WordHover text={cueparcel.summary} /></p><ol className="v2-process" aria-label="CueParcel workflow">{["Pick", "Cart", "Recipe", "TaskSpec", "Receipt"].map((step, index) => <li key={step}><small>0{index + 1}</small>{step}</li>)}</ol><CaseLink slug={cueparcel.slug} /></div>
          <div className="v2-case-media v2-reveal"><ProjectImage src={cueparcel.image} alt={cueparcel.imageAlt} caption={cueparcel.imageCaption} /></div>
        </div>
      </article>
      <article className="v2-case v2-field-host" aria-labelledby="agent-title">
        <SectionPattern kind="agent" />
        <div className="v2-frame v2-case-inner">
          <div className="v2-case-count"><span>03 / 04</span><span>{agentStudio.kicker}</span></div>
          <div className="v2-case-copy v2-reveal"><h3 id="agent-title">Agent <em>Studio</em></h3><p className="v2-case-lead"><WordHover text={agentStudio.summary} /></p><div className="v2-sequence" aria-label="Agent Studio architecture">Runtime → Tool → RAG → Memory → Trace → Evaluation</div><CaseLink slug={agentStudio.slug} /></div>
          <div className="v2-case-media v2-reveal"><ProjectImage src={agentStudio.image} alt={agentStudio.imageAlt} caption={agentStudio.imageCaption} /></div>
        </div>
      </article>
      <article className="v2-case v2-case-ochre v2-case-reverse v2-field-host" aria-labelledby="skin-title">
        <SectionPattern kind="skin" />
        <div className="v2-frame v2-case-inner">
          <div className="v2-case-count"><span>04 / 04</span><span>{skinLesion.kicker}</span></div>
          <div className="v2-case-copy v2-reveal"><h3 id="skin-title">Skin Lesion <em>AI Platform</em></h3><p className="v2-case-lead"><WordHover text={skinLesion.summary} /></p><div className="v2-metrics" aria-label="Stored evaluation results">{skinLesion.metrics?.map((metric) => <div key={metric.label}><strong>{metric.value}</strong><span>{metric.label}</span></div>)}</div><p>Engineering practice. <strong>Not a clinical diagnosis product.</strong></p><CaseLink slug={skinLesion.slug} /></div>
          <div className="v2-case-media v2-reveal"><ProjectImage src={skinLesion.image} alt={skinLesion.imageAlt} caption={skinLesion.imageCaption} /></div>
        </div>
      </article>
    </section>
    ...
    <section id="education" className="v2-education v2-field-host" aria-labelledby="education-title" lang="zh-CN">
      ...
      <div className="v2-frame">
        <div className="v2-education-meta v2-reveal"><span>02 / 教育经历</span><span>2023.09 — 2027.06</span></div>
        <div className="v2-education-spread">
          ...
          <div className="v2-education-copy v2-reveal"><span className="v2-registration" aria-hidden="true">┼</span><h2 id="education-title">华南农业大学</h2><p>信息管理与信息系统</p><div className="v2-education-data"><span>本科</span><span>2023.09 — 2027.06</span><span>2027届</span></div></div>
        </div>
      </div>
    </section>
    ...
    <section id="profile" className="v2-method v2-field-host" aria-labelledby="profile-title">
      ...
      <div className="v2-frame"><ChapterMark number="03" title="Method & experience" />
        <div className="v2-method-grid">
          ...
          ].map(([number, title, description]) => <div key={number}><small>{number}</small><strong>{title}</strong><p><WordHover text={description} /></p></div>)}</div>
        </div>
    ...
    <section id="visual" className="v2-visual v2-field-host" aria-labelledby="visual-title">
      ...
      <div className="v2-frame"><ChapterMark number="04" title="Visual practice" />
        <div className="v2-visual-heading v2-reveal"><h2 id="visual-title">The line <em>remains.</em></h2><p><WordHover text="One original drawing. Hand-drawn manga line work meets digital fragmentation." /></p></div>
    ...
    <section id="contact" className="v2-contact v2-field-host" aria-labelledby="contact-title">
      ...
      <div className="v2-frame"><ChapterMark number="05" title="Contact" />
        <div className="v2-contact-body v2-reveal"><p>For work where the evidence matters.</p><h2 id="contact-title">kallist<span aria-hidden="true">.</span></h2><div className="v2-contact-links"><a href="mailto:2441397782@qq.com">2441397782@qq.com <span aria-hidden="true">↗</span></a><a href="https://github.com/kallist" target="_blank" rel="noopener noreferrer">GitHub <span aria-hidden="true">↗</span></a><Link href="/resume/">Resume <span aria-hidden="true">↗</span></Link></div></div>

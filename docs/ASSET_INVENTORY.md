...
Initial assets were copied on 2026-09-23; the fifth gallery image was supplied on 2026-09-26. Project images are originals from the cited public repositories at pinned commits, with no face or product UI reconstruction.
...
| `assets/gallery/*.png` | Five images supplied by the user for V2.1 | Original files retained outside static export; authorship and date are not inferred |
| `public/gallery/*.webp` | Encoded from the five supplied PNGs with Pillow WebP quality 92, preserving dimensions | Five distinct supplied images: the earlier four character illustrations and comic drawings, plus the 1916×821 riverside illustration in gallery position 05 |
...
| `public/graphics/ascii-tree-{near,middle,far}.svg` | Repo-native glyph density composition from the V2.1 references supplied by the user | Deterministic, fine branch, crown and trunk cells form the three depth layers (5,323 glyphs total); static SVG files keep the glyph nodes out of the homepage DOM. CSS provides slow depth drift, disabled for reduced motion. |

...
| `public/gallery/*.webp` | Encoded from those four supplied PNGs with Pillow WebP quality 92, preserving dimensions | The visual band uses four distinct images: two landscape character illustrations and two portrait comic drawings |
...
| `public/graphics/ascii-tree-{near,far}.svg` | Repo-native glyph density composition from the V2.1 reference supplied by the user | Deterministic branch and trunk cells form the tree; static SVG files keep thousands of glyph nodes out of the homepage DOM. CSS provides slow depth drift, disabled for reduced motion. |
